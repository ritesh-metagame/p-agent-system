    ...
    useEffect(() => {
      const checkMobileNumberUnique = async () => {
          ...
          try {
            ...
            const response = await axios.get(`/api/check-mobileNumber-unique?mobileNumber=${debouncedMobileNumber}`);
            setMobileUniqueMessage(response.data.message);
          } catch (error) {
            if (axios.isAxiosError(error) && error.response) {
              setMobileUniqueMessage(error.response.data?.message ?? 'Error checking mobile number uniqueness');
            } else {
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>Bank Account Number For Settlement</FormLabel>
                    <FormControl>
                      <Input placeholder="Bank Account Number For Settlement" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
