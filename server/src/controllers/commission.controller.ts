...
class CommissionController {
    ...
    public static async markSettledCommission(
        ...
        try {
            const {ids, childrenCommissionIds, referenceId} = req.body as any;
            const user = req.user as any
            const roleDao = new RoleDao();
            const role = await roleDao.getRoleById(user.roleId)
            const commissionService = Container.get(CommissionService);
            const result = await commissionService.markCommissionSummaryStatus(ids, childrenCommissionIds, role.name as UserRole, referenceId);
