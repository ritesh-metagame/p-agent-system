...
class Server {
  ...
  const rawRows: any = await prisma.$queryRawUnsafe(`
    SELECT * FROM bets
    WHERE time_of_bet BETWEEN '2025-05-26 00:00:00' AND '2025-06-23 23:59:59'
  `);
  ...
  for (const row of rawRows) {
    ...
    const normalizedPlatform =
      platformType === "egames"
        ...
        ? "Sports Betting"
        : platformType === "specialtyrng"
        ? "Speciality Games - RNG"
        : platformType === "specialtytote"
    ...
    let betAfterRefund;
    if (platformType === "sportsbet" || platformType === "specialtytote") {
    ...
    const maName = await prisma.user.findFirst({ where: { id: maId } });
    ...
    if (maId) {
      ...
      const ownerUser = await prisma.user.findFirst({ where: { id: ownerId } });
      ...
      if (ownerId) {
        ...
        ownerPercentage = new Decimal(ownerCommissionRecord?.commissionPercentage || 0);
    ...
    const transaction = {
      ...
      betAmount:
        platformType === "sportsbet" || platformType === "specialtytote"
          ? betAfterRefund
          : new Decimal(row["bet_amount"] || 0),
