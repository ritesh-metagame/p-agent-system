...
class CommissionService {
    ...
    try {
      ...
      if (roleName === UserRole.SUPER_ADMIN) {
        ...
        for (const [_, groupSummaries] of groupedSettledSummaries.entries()) {
          ...
          if (total >= 0) {
            filteredSettledSummaries =
              filteredSettledSummaries.concat(groupSummaries);
          }
        }
      ...
      const totalCommission = commissionSummaries.reduce(
        (acc, curr) => (curr.netCommissionAvailablePayout += acc),
        0
      );
      console.log(
        `Commission summaries from ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++`,
        totalCommission
      );
      ...
      for (const summary of settledSummaries) {
        console.log(
          `--------------------------------------------------Net commission available payout: `,
          summary.netCommissionAvailablePayout
        );
      ...
      console.log(
        `--------------------------------------------------Pending commission available payout: `,
        totalPending
      );
  ...
  public async getTotalBreakdown(userId: string, roleName: string) {
    try {
      ...
      // 🔹 Enhanced Sum Logic Per Downline Chain
      const downlineCommissionSums = new Map<
        string,
        {
          sumWithNegatives: Record<string, number>;
          sumWithoutNegatives: Record<string, number>;
          immediateChildId: string;
        }
      >();

      let finalTotalMaxPro = 0;

      // Get cycle dates
      const cycleDates = await this.getPreviousCompletedCycleDates("E-Games");
      const cycleStartDate = cycleDates.cycleStartDate;
      const cycleEndDate = cycleDates.cycleEndDate;

      if (roleName === UserRole.SUPER_ADMIN) {
        await this.handleSuperAdminBreakdown(
          userId,
          categoryData,
          ownCommissionData,
          downlineCommissionSums,
          cycleStartDate,
          cycleEndDate
        );
      } else if (roleName === UserRole.OPERATOR) {
        await this.handleOperatorBreakdown(
          userId,
          categoryData,
          ownCommissionData,
          downlineCommissionSums,
        ...
        await this.handlePlatinumBreakdown(
          userId,
          categoryData,
          ownCommissionData,
          downlineCommissionSums,
        ...
        await this.handleGoldenBreakdown(
          userId,
          categoryData,
          cycleStartDate,
          cycleEndDate
      ...
      // Calculate finalTotalMaxPro from immediate children's sumWithoutNegatives
      finalTotalMaxPro = this.calculateFinalTotalMaxPro(downlineCommissionSums);

      // Process category totals and return formatted response
      return this.formatBreakdownResponse(
        categoryData,
        ownCommissionData,
        cycleStartDate,
        cycleEndDate,
  ...
  private async handleSuperAdminBreakdown(
    userId: string,
    categoryData: any,
    ownCommissionData: any,
    downlineCommissionSums: Map<string, any>,
    cycleStartDate: Date,
    cycleEndDate: Date
  ) {
    // Get immediate children (Operators)
    const operators = await prisma.user.findMany({
      where: {
        parentId: userId,
        role: { name: UserRole.OPERATOR },
    ...
    // For each operator, get their downline hierarchy
    for (const operator of operators) {
      const operatorDownline = await this.getOperatorDownlineHierarchy(
        operator.id
      );

      // Initialize sums for this operator
      const operatorSums = {
        sumWithNegatives: {} as Record<string, number>,
        sumWithoutNegatives: {} as Record<string, number>,
        immediateChildId: operator.id,
      };

      // Initialize category sums
      Object.keys(categoryData).forEach((category) => {
        operatorSums.sumWithNegatives[category] = 0;
        operatorSums.sumWithoutNegatives[category] = 0;
      });

      // Get commission data for this operator's entire downline
      const allDownlineIds = [
        operator.id,
        ...operatorDownline.platinumIds,
        ...operatorDownline.goldenIds,
      ];

      for (const category of Object.keys(categoryData)) {
        const commissionData = await this.getCommissionDataForCategory(
          allDownlineIds,
          category,
          cycleStartDate,
          cycleEndDate,
        ...
        const categorySum = commissionData.reduce(
          (sum, record) => sum + (record.netCommissionAvailablePayout || 0),
          0
        );

        operatorSums.sumWithNegatives[category] = categorySum;
        operatorSums.sumWithoutNegatives[category] =
          categorySum < 0 ? 0 : categorySum;

        // Add data to categoryData if sum is positive
        if (categorySum > 0) {
          categoryData[category].pending.push(...commissionData);
        }
      }

      // Store the operator's sums
      downlineCommissionSums.set(operator.id, operatorSums);
    }

    // Handle settled data
    await this.handleSettledData(
  ...
  private async handleOperatorBreakdown(
    userId: string,
    categoryData: any,
    ownCommissionData: any,
    downlineCommissionSums: Map<string, any>,
    ...
    // Get immediate children (Platinums)
    const platinums = await prisma.user.findMany({
      where: {
        parentId: userId,
        role: { name: UserRole.PLATINUM },
    ...
    await this.calculateOwnCommission(
      userId,
      ownCommissionData,
      cycleStartDate,
      cycleEndDate
    ...
    // For each platinum, get their downline hierarchy
    for (const platinum of platinums) {
      const platinumDownline = await this.getPlatinumDownlineHierarchy(
        platinum.id
      );
      ...
      const platinumSums = {
        sumWithNegatives: {} as Record<string, number>,
        sumWithoutNegatives: {} as Record<string, number>,
        immediateChildId: platinum.id,
      };

      // Initialize category sums
      Object.keys(categoryData).forEach((category) => {
        platinumSums.sumWithNegatives[category] = 0;
        platinumSums.sumWithoutNegatives[category] = 0;
      });

      const allDownlineIds = [platinum.id, ...platinumDownline.goldenIds];
      ...
      for (const category of Object.keys(categoryData)) {
        ...
        platinumSums.sumWithNegatives[category] = categorySum;
        platinumSums.sumWithoutNegatives[category] =
          categorySum < 0 ? 0 : categorySum;
      ...
      downlineCommissionSums.set(platinum.id, platinumSums);
    }

    // Handle settled data
    await this.handleSettledData(
      platinums.map((p) => p.id),
      categoryData,
      UserRole.OPERATOR
    );
  }

  private async handlePlatinumBreakdown(
    userId: string,
    categoryData: any,
    ownCommissionData: any,
    downlineCommissionSums: Map<string, any>,
    ...
    // Get immediate children (Goldens)
    const goldens = await prisma.user.findMany({
      where: {
        parentId: userId,
        role: { name: UserRole.GOLDEN },
    ...
    for (const golden of goldens) {
      const goldenSums = {
        sumWithNegatives: {} as Record<string, number>,
        sumWithoutNegatives: {} as Record<string, number>,
        immediateChildId: golden.id,
      };

      // Initialize category sums
      Object.keys(categoryData).forEach((category) => {
        goldenSums.sumWithNegatives[category] = 0;
        goldenSums.sumWithoutNegatives[category] = 0;
      });
      ...
      for (const category of Object.keys(categoryData)) {
        ...
        goldenSums.sumWithNegatives[category] = categorySum;
        goldenSums.sumWithoutNegatives[category] =
          categorySum < 0 ? 0 : categorySum;
      ...
      downlineCommissionSums.set(golden.id, goldenSums);
    }
  ...
  private async handleGoldenBreakdown(
    userId: string,
    categoryData: any,
    cycleStartDate: Date,
    cycleEndDate: Date
  ) {
    // Golden users don't have downlines, so we just get their own data
    for (const category of Object.keys(categoryData)) {
      const commissionData = await this.getCommissionDataForCategory(
        [userId],
        category,
      ...
      categoryData[category].pending.push(...commissionData);
      // No settled data for golden users
      categoryData[category].settled = [];
    }
  }

  private async getOperatorDownlineHierarchy(operatorId: string) {
    const platinums = await prisma.user.findMany({
      where: {
        parentId: operatorId,
        role: { name: UserRole.PLATINUM },
    ...
    return {
      platinumIds,
      goldenIds: goldens.map((g) => g.id),
    };
  }

  private async getPlatinumDownlineHierarchy(platinumId: string) {
    const goldens = await prisma.user.findMany({
      where: {
        parentId: platinumId,
        role: { name: UserRole.GOLDEN },
  ...
  private async getCommissionDataForCategory(
    userIds: string[],
    category: string,
    cycleStartDate: Date,
    cycleEndDate: Date,
    ...
    if (roleName === UserRole.SUPER_ADMIN) {
      whereConditions.settledBySuperadmin = false;
      whereConditions.settledStatus = "N";
    } else if (roleName === UserRole.OPERATOR) {
      whereConditions.settledByOperator = false;
      whereConditions.settledStatus = "N";
    } else if (roleName === UserRole.PLATINUM) {
      whereConditions.settledByPlatinum = false;
      whereConditions.settledStatus = "N";
    } else if (roleName === UserRole.GOLDEN) {
      whereConditions.settledStatus = { in: ["Y", "N"] };
    }
  ...
  private async calculateOwnCommission(
    userId: string,
    ownCommissionData: any,
    cycleStartDate: Date,
    cycleEndDate: Date
    ...
    for (const category of Object.keys(ownCommissionData)) {
      const commissionData = await prisma.commissionSummary.findMany({
        where: {
          userId: userId,
          categoryName: category,
          createdAt: {
      ...
      const commission = commissionData.reduce(
        (acc, curr) => acc + (curr.parentCommission || 0),
        0
      );
  ...
  private async handleSettledData(
    userIds: string[],
    categoryData: any,
    roleName: string
  ) {
  ...
  private calculateFinalTotalMaxPro(
    downlineCommissionSums: Map<string, any>
  ): number {
    let total: any = 0;

    for (const [childId, sums] of downlineCommissionSums.entries()) {
      // Sum all categories for this immediate child using sumWithoutNegatives
      const childTotal = Object.values(sums.sumWithoutNegatives).reduce(
  ...
  private formatBreakdownResponse(
    categoryData: any,
    ownCommissionData: any,
    cycleStartDate: Date,
    cycleEndDate: Date,
    roleName: string,
    finalTotalMaxPro: number
  ) {
    ...
    this.processCategoryData(
      categoryData["E-Games"].pending,
      categoryTotals.pending.egames,
      categoryTotals.pending
    );
    this.processCategoryData(
      categoryData["Sports Betting"].pending,
      categoryTotals.pending.sports,
      categoryTotals.pending
    );
    this.processCategoryData(
      categoryData["Speciality Games - RNG"].pending,
      categoryTotals.pending.specialtyGamesRng,
      categoryTotals.pending
    );
    this.processCategoryData(
      categoryData["Speciality Games - Tote"].pending,
      categoryTotals.pending.specialtyGamesTote,
      categoryTotals.pending
    );
    ...
    if (categoryData["E-Games"].settled) {
      this.processCategoryData(
        categoryData["E-Games"].settled,
        categoryTotals.settled.egames,
        categoryTotals.settled
      );
    }
    if (categoryData["Sports Betting"].settled) {
      this.processCategoryData(
        categoryData["Sports Betting"].settled,
        categoryTotals.settled.sports,
        categoryTotals.settled
      );
    }
    if (categoryData["Speciality Games - RNG"].settled) {
      this.processCategoryData(
        categoryData["Speciality Games - RNG"].settled,
        categoryTotals.settled.specialtyGamesRng,
        categoryTotals.settled
      );
    }
    if (categoryData["Speciality Games - Tote"].settled) {
      this.processCategoryData(
        categoryData["Speciality Games - Tote"].settled,
        categoryTotals.settled.specialtyGamesTote,
        categoryTotals.settled
      );
    }

    // Calculate final amounts
    const pendingOwnCommission: any = Object.values(ownCommissionData).reduce(
      (sum: any, category: any) => sum + category.pending,
      0
    );

    const totalPendingGrossCommission: any =
      categoryTotals.pending.egames.amount +
      categoryTotals.pending.sports.amount +
      categoryTotals.pending.specialtyGamesRng.amount +
      categoryTotals.pending.specialtyGamesTote.amount;
    ...
    const totalPendingCommission =
      totalPendingGrossCommission + pendingOwnCommission;
    const finalPendingAmount: any =
      totalPendingCommission - pendingOwnCommission;
    const transferableAmount = finalPendingAmount >= 0 ? finalPendingAmount : 0;
    ...
    return {
      ...
      finalTotalMaxPro, // New field with calculated value
      downlineBreakdown: {
        totalWithNegatives: totalPendingGrossCommission + pendingOwnCommission,
        totalWithoutNegatives: finalTotalMaxPro,
      },
      rows: [
        {
          label: "Total EGames",
        ...
        {
          label: "Gross Commissions",
          pendingSettlement: totalPendingCommission,
          settledAllTime: totalSettledGrossCommission,
        },
        ...(roleName !== UserRole.GOLDEN
          ? [
          ...
          ? [
              {
                ...
                pendingSettlement: transferableAmount,
                settledAllTime: 0,
                note: "(Commission Available for Payout, negative values treated as 0)",
              },
            ]
          : []),
      ],
  ...
  private processCategoryData(
    ...
    data.forEach((summary) => {
      categoryTotal.amount += summary.netCommissionAvailablePayout || 0;
      categoryTotal.grossCommission += summary.grossCommission || 0;
      overallTotal.totalGrossCommission += summary.grossCommission || 0;
      overallTotal.totalPaymentGatewayFees += summary.paymentGatewayFee || 0;
      overallTotal.netCommissionPayout +=
        summary.netCommissionAvailablePayout || 0;
    });
  }
  ...
  public async getPaymentGatewayFeesBreakdown(
    ...
    console.log(
      `Getting payment gateway fees for date range: ${cycleStartDate} to ${cycleEndDate}`
    );
  ...
  public async getPendingSettlements(userId: string, roleName: string) {
    try {
      ...
      //   ownCommission = Object.values(ownCommissionData).reduce((acc:number, curr:number) => acc + curr, 0);
      ...
      for (const category of categories) {
        ...
        const isSettledBySuperAdmin =
          ownCommissionData[category].settledBySuperAdmin;
        const isSettledByOperator =
          ownCommissionData[category].settledByOperator;
        const isSettledByPlatinum =
          ownCommissionData[category].settledByPlatinum;
        ...
        commissionSummaries = shouldConcat
          ? commissionSummaries.concat(summaries)
          : commissionSummaries.concat([]);
        restCommissionSummaries = shouldConcat
          ? restCommissionSummaries.concat(restSummaries)
          : restCommissionSummaries.concat([]);
      ...
      const rows = await Promise.all(
        Object.values(summariesByUserId).map(async (userSummaries) => {
          ...
          const ownCommission = (userSummaries as any[]).reduce(
            (acc, cur) => (acc += cur.parentCommission || 0),
            0
          );
          ...
          const totalSportsBettingCommissions =
            summariesByUser.totalSportsBettingCommissions;
          const totalSpecialtyGamesRNGCommissions =
            summariesByUser.totalSpecialtyGamesRNGCommissions;
          const totalSpecialtyGamesToteCommissions =
            summariesByUser.totalSpecialtyGamesToteCommissions;

          const grossCommissions =
            totalEgamesCommissions +
          ...
          return {
            ...
            totalSportsBettingCommissions: totalSportsBettingCommissions,
            totalSpecialtyGamesRNGCommissions:
              totalSpecialtyGamesRNGCommissions,
            totalSpecialtyGamesToteCommissions:
              totalSpecialtyGamesToteCommissions,
        ...
        return {
          ...
          fields: [
            ...
            },
            {
              ...
              pendingSettlement:
                roleName === UserRole.OPERATOR
                  ? commissionPending + ownCommission
                  : commissionPending,
  ...
  private async getCommissionSummariesForUserIds(
    cycleStartDate: Date,
    cycleEndDate: Date,
    category: string,
    userIds: string[]
